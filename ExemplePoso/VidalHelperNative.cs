...
namespace ExemplePoso
{
    class VidalHelperNative : ExemplePoso.IVidalHelper 
    {
      ...
      public string AnalysisService_patientFromObjToJson(DateTime dateTime, GenderDto? gender, int weight, int height, int amen, BreastFeedingDto? breast, int creatin, HepaticInsufficiencyDto? hepatic,List<int> allergies, List<int> molecules, List<int> cims)
       ...
       {
           VidalAPI.Domain.DocumentList documents = vidalProduct.GetService<VidalAPI.Services.DocumentService>().GetPgrForProductId((int)nullable);
           List<string> url = new List<String>();
           foreach (VidalAPI.Domain.Document doc in documents)
           {
           ...
           VidalAPI.Domain.DocumentList documents2 = vidalProduct.GetService<VidalAPI.Services.DocumentService>().GetFitForProductId((int)nullable);
           foreach (VidalAPI.Domain.Document doc in documents2)
           {
               if (doc.UrlAsString != null)
               {
                   url.Add(doc.UrlAsString);
           ...
           VidalAPI.Domain.DocumentList documents3 = vidalProduct.GetService<VidalAPI.Services.DocumentService>().GetBumForProductId((int)nullable);
           foreach (VidalAPI.Domain.Document doc in documents3)
           {
               if (doc.UrlAsString != null)
               {
                   url.Add(doc.UrlAsString);
